    ...
    def StartGame(self):
        ...
        while not self.state == GameState.GAME_OVER:
            ...
            if self.state == GameState.WHITE_TURN: # human's turn
                ...
                jumped, crowned = self.board.Move(PlayerColor.WHITE, human_move[0], human_move[1])
                # house cleaning
                if jumped is not None:
                    ...
                    if not crowned:
                        ...
                        for key, val in self.board.tiles.iteritems():
                            ...
                            if self.board.ValidJump(PlayerColor.WHITE, start, end)[0]:
                                available_jumps.append((start, end))
                        if len(available_jumps) > 0:
                            print "There is another jump available. TAKE IT!"
                            continue
        ...
        for start in pieces:
            for key, val in self.board.tiles.iteritems():
                ...
                if self.board.ValidJump(PlayerColor.RED, start, end)[0]:
                    available_jumps.append((start, end))
        if len(available_jumps) > 0:
            move = random.randint(0, len(available_jumps) - 1)
            captured = self.board.ValidJump(PlayerColor.RED, available_jumps[move][0], available_jumps[move][1])[1]
            return available_jumps[move], captured
        else:
            move = random.randint(0, len(available_moves) - 1)
            return available_moves[move], None
...
class CheckersBoard:
    ...
    def Move(self, color, start, end):
        ...
        if color == PlayerColor.WHITE and self.tiles.get(end).row == '6':
            ...
            crown = True
        return jumped[1], crown # Return tile key of jumped piece, will be None is not jumped. Also return whether the move was a crowning move (True/False)
